...
## 10. Monorepo y MCP (Etapa 5)

El proyecto se reestructuró como **monorepo** (npm workspaces + Turborepo) para
alojar varias aplicaciones y compartir lógica sin duplicar código:

- `apps/web` — esta aplicación Next.js (sin cambios funcionales).
- `apps/mcp` — servidor **MCP** de ReadHub (ver `apps/mcp/README.md`).
- `packages/*` — código compartido: `@readhub/types`, `@readhub/config`,
  `@readhub/database` (cliente admin + acceso a datos Node), `@readhub/ai`
  (embeddings Voyage, prompts y llamada al LLM), `@readhub/shared` (servicios
  RAG: embedding, vector-search, context-builder, chat).

La web consume los paquetes vía `transpilePackages`; el servidor MCP los ejecuta
con `tsx`. Ambos reutilizan **los mismos servicios** — el RAG no está duplicado.

**Servidor MCP** (STDIO): expone Tools (buscar/listar/obtener artículos,
búsqueda semántica, responder con RAG, comparar, extraer temas, contexto de
investigación), Resources (`readhub://articles`, `article/{id}`, `authors`,
`stats`, `info`, `categories`) y Prompts (resumir, explicar, comparar, generar
preguntas, extraer conceptos). Lo consume un cliente MCP (Claude Desktop / Code /
Inspector).

**Skill "Asistente del Escritor"** (`skills/asistente-del-escritor/`): Skill de
Claude que orquesta las Tools/Prompts del MCP para acompañar al escritor durante
la creación, revisión y prepublicación de un artículo.
